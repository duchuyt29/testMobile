...
class CrmDetailDetailContactController extends BaseController {
  ...
  Future<void> onViewFormInformation() async {
    await Get.toNamed(CRMRoutes.CRM_CONTACT_FORM_UPDATE_INFORMATION,
  ...
  Future<void> onViewFormAddressContact(String type) async {
    await Get.toNamed(CRMRoutes.CRM_CONTACT_FORM_ADDRESS_CONTACT, arguments: {
      CONTACT_INFO: contact.value,
      CONTACT_ADDRESS_TYPE: type,
    })?.then((value) {
      if (value != null) {
        _loadData();
      }
  ...
  void onBack() {
    Get.back(result: contact.value);
  }

  Future<void> onPhoneAction() async {
    final phoneUri =
        Uri(scheme: 'tel', path: contact.value?.contactPhone ?? '');
    await EasyLoading.show();
